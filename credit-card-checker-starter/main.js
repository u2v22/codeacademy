...
function validateCred(array){
  let sum = 0;
  // Fail fast
  if (array.length > 16 || array.length < 15){
    console.log (`${array.join('')} - Credit card is invalid FF`);
    return 'invalid';
  ...
  array.forEach(function(item, index){
    ...
    if (isDivByTen(sum)){
      console.log(`${array.join('')} - Credit card is valid`);
      return 'valid';
    } else {
      console.log(`${array.join('')} - Credit card is invalid`);
      return 'invalid';
    }
}
...
function findInvalidCards(batch){
  const validCards = [];
  const invalidCards = [];
  batch.forEach(function(cardNum) {
    if(validateCred(cardNum) === 'valid'){
...
function idInvalidCardCompanies(batch){
  ...
  allInvalidCompanies = findInvalidCards(batch);

  allInvalidCompanies.forEach(function(cardNum){
    if(cardNum[0] === 0){
    ...
    if (cardNum[0] === 3){
      ...
      console.log(`Company not found for card ${cardNum.join(' ')}`);
    }
  });
